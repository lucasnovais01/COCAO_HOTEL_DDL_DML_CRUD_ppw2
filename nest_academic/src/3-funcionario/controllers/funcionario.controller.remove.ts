...
export class FuncionarioControllerRemove {
	...
	async remove(@Req() req: Request, @Param('id') id: string): Promise<Result<null>> {
		await this.funcionarioServiceRemove.remove(Number(id));

		return MensagemSistema.showMensagem(HttpStatus.NO_CONTENT, 'Funcionário removido com sucesso', null, ROTA.FUNCIONARIO.DELETE, null);
	}
}
